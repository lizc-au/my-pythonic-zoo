"""
Python implementation used by the Factory Pattern example.

Python is a concrete animal class. It provides the ``speak()`` behaviour
...
class Python:
    """
    Represent a python satisfying this example's ``Animal`` contract.

    See ``factory.animals.animal.Animal`` for scope and related use cases.
    """

    def speak(self) -> None:
        """Display one sound made by this python."""
        print("Hiss!")
